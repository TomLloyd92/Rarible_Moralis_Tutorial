...
contract MorarableMarketContract {
    ...
    modifier ItemExists(uint256 id){
        require (id < itemsForSale.length && itemsForSale[id] == id, "Could not Find Item");
        _;
    }
    ...
    function addItemToMarket(uint256 tokenId, address tokenAddress, uint256 askingPrice) 
    OnlyItemOwner(tokenAddress, tokenId) 
    HasTransferApproval(tokenAddress,tokenId ) 
    external returns(uint256)
    {
        //Check Item is not already on sale
        ...
        assert(itemsForSale[newItemId] == newItemId);
    ...
    function buyItem(uint256 id) payable external ItemExists(id) IsForSale(id) HasTransferApproval(itemsForSale[id].tokenAddress, itemsForSale[id].tokenId)
    {
        ...
        IERC721(itemsForSale[id].tokenAddress).safeTransferFrom(itemsForSale[id].seller, msg.sender, tokenId)
